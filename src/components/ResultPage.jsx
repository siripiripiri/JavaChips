  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const diagnosisMatch = text.match(/Diagnosis:(.*?)Remedies:/s);
        const remediesMatch = text.match(/Remedies:(.*)/s);

        let diagnosisText = diagnosisMatch ? diagnosisMatch[1].trim() : '';
        let remediesText = remediesMatch ? remediesMatch[1].trim() : '';

        // Remove unwanted characters like `#`
        diagnosisText = diagnosisText.replace(/#/g, '');
        remediesText = remediesText.replace(/#/g, '');
  
          remediesText = remediesText.replace(/\*(.*?)\*/, (match, p1) => {
            return `<br>*${p1}*`;
          });

        // Convert `**text**` to `<b>text</b>` for JSX
        diagnosisText = diagnosisText.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>');
        remediesText = remediesText.replace(/\*\*(.*?)\*\*/g, '<br><b>$1</b>');
        remediesText = remediesText.replace(/\*/g, '');


        setDiagnosis(diagnosisText);
        setRemedies(remediesText);
      } catch (error) {
    ...
    <>
      <nav id='sidebar'>
        <div className='sidebar-header'>
          <svg className='svgs' xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 68 68" fill="none">
      ...
      <nav id='rightsidebar'>
        <div className='sidebar-header'>
          ...
          <img height="40" width="40" className='profilepic' src={auth?.currentUser?.photoURL} alt="" />
        </div>
      ...
      <div className="result-page">
        <div className="result-page__container">
          {/* <div className="result-page__container__title">
            <h1 className='result-title'>This is your diagnosis:</h1>
          </div> */}
          <div className='diagnosis-container results-container'>
            <h1 className='diagnosis-title results-title'>Diagnosis</h1>
            <p className='diagnosis-text results-title' dangerouslySetInnerHTML={{ __html: diagnosis }} />
          </div>
          <div className='remedies-container results-container'>
            <h1 className='remedies-title results-title'>Remedies</h1>
            <p className='remedies-text' dangerouslySetInnerHTML={{ __html: remedies }} />
          </div>
        </div>
      </div>
    </>
